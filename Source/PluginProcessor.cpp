...
void SonicDoctorAudioProcessor::analyseSpectrumIntoProfile()
{
    ...
    for (auto& band : snapshot.bands)
    {
        band.referenceDb = getSourceTypeConfig (snapshot.sourceType).referenceDb[refIndex++];
...
void SonicDoctorAudioProcessor::finalizeCapture()
{
    ...
    for (int i = 0; i < SoundProfile::numBands; ++i)
    {
        ...
        band.energyDb = capBandCount[i] > 0
            ...
            : -100.0f;
        band.referenceDb = getSourceTypeConfig (result.sourceType).referenceDb[i];
        band.deviationDb = band.energyDb - band.referenceDb;
    }
    ...
    result.sibilanceExcessDb = result.sibilanceLevelDb - (-30.0f);
