...
std::vector<Recommendation> KickForgeEQRecommender::generate (const SoundProfile& profile) const
{
    ...
    // Banda "Sub Rumble" (30Hz) — ruido de manejo/HVAC
    if (profile.rumbleLevelDb > -45.0f)
    {
        out.push_back ({ "Sub Rumble (30Hz)", "Corte -4 a -6 dB (o activar como HPF)",
            "Se detectó energía de rumble/ruido de baja frecuencia (" + juce::String (profile.rumbleLevelDb, 1) + " dB) por debajo de 40Hz." });
    }
