...
std::vector<juce::String> RecommendationEngine::generateGeneralDiagnostics (const SoundProfile& profile)
{
    ...
    // --- Dinámica ---
    if (profile.crestFactorDb < 6.0f && profile.peakDb > -60.0f)
    ...
    // --- Sibilancia ---
    if (profile.sibilanceExcessDb > 3.0f)
        lines.push_back ("Sibilancia elevada (" + juce::String (profile.sibilanceExcessDb, 1) + " dB sobre lo típico) — considera un de-esser.");

    // --- Rumble ---
    if (profile.rumbleLevelDb > -45.0f)
        lines.push_back ("Ruido de baja frecuencia (rumble) presente — considera un high-pass filter.");
