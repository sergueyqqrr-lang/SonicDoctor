...
std::vector<juce::String> RecommendationEngine::generateGeneralDiagnostics (const SoundProfile& profile)
{
    ...
    if (profile.rumbleLevelDb > cfg.rumbleThresholdDb)
        lines.push_back ("Ruido de muy baja frecuencia (<30Hz) presente — esto está por debajo de lo que produce cualquier instrumento musical, probablemente sea ruido de manejo/HVAC. Considera un high-pass filter.");
