...
struct SpectralBand
{
    ...
    float referenceDb = -100.0f;   // energía "típica"/esperada para el tipo de fuente elegido
    float deviationDb = 0.0f;      // energyDb - referenceDb (positivo = exceso, negativo = falta)
};

enum class SourceType { Generico, Voz, Kick, Bajo, MezclaCompleta };
...
struct SoundProfile
{
    ...
    float sibilanceExcessDb = 0.0f;    // cuánto excede lo típico
    float rumbleLevelDb = -100.0f;     // energía por debajo de 40Hz (ruido de manejo, HVAC, etc.)
